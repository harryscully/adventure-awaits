...
export function Menu({ order, solved, onOpen, onFinal }: Props) {
  const allSolved = PUZZLE_LIST.every((p) => solved[p.id]);
  const count = PUZZLE_LIST.filter((p) => solved[p.id]).length;

  return (
    <div className="min-h-svh px-5 py-10 flex flex-col items-center">
      <div className="max-w-xl w-full text-center fade-up">
        <p className="typewriter text-xs tracking-[0.3em] text-ink-soft uppercase">
          from Frog
        </p>
        <h1 className="hand text-5xl sm:text-6xl text-primary mt-1">{COPY.menu.title}</h1>
        <p className="text-base text-ink-soft mt-2">{COPY.menu.sub}</p>
        <p className="typewriter text-xs tracking-widest text-ink-soft uppercase mt-3">
            ...
            <li key={p.id}>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {isSolved && <span className="text-primary text-xl">✓</span>}
                </div>
                <p className="hand text-3xl text-ink mt-1">
                  {isSolved ? p.answer.toUpperCase() : "—————"}
                </p>
                <p className="text-sm text-ink-soft mt-1">{p.teaser}</p>
              </button>
            </li>
          );
        })}
      ...
      <button
        onClick={onFinal}
        disabled={!allSolved}
        className={`mt-8 px-7 py-3 rounded-full text-base font-medium shadow-lg fade-up ${
          allSolved
